...
export async function PUT(
  ...
  try {
    ...
    // Преобразуем все строковые даты в объекты Date (универсально)
    updatedData = updatedData.map((item: any) => {
      const newItem = { ...item };
      for (const key in newItem) {
        // Если значение похоже на дату (ISO-строка), преобразуем в Date
        if (
...
export async function POST(
  ...
  try {
    ...
    if (!newItem.id) delete newItem.id; // <-- Удаляем id, если он пустой
